...
- **路径引用**：文档、说明、变更摘要里的文件路径，一律以**当前工作目录**为基准写相对路径，例如 `beex-service/seahub-core/...`；不要写 `/Users/xxx/...`、`/home/xxx/...` 这类个人机器绝对路径。
...
7. **路径必须相对当前工作目录**。除非用户明确要求本机路径，否则所有文件引用、代码位置、文档链接都写相对路径，避免把个人电脑路径写进团队资料。
